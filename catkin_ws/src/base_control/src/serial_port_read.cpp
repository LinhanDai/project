...
bool imu_data_processing(const Imu_t *imu)
{
...
void *imu_serial_recv_thread(void *arg)
{
    ...
    while(1)
    {
        ...
        try
        {
            imu_serial_port.ReadByte(ch, ms_timeout);
            printf("%x ",ch );
        }
        catch (ReadTimeout)
        {
...
int main(int argc, char **argv)
{
    ...
    //std::vector<std::string> port_list =serial_port.GetAvailableSerialPorts();
    //std::cout << port_list[0]<<std::endl;
    //std::string serial_str ("USB");
    //for(int i=0;i<port_list.size();i++)
    //{
      //  std::size_t found = port_list[i].find(serial_str);
        // if (found!=std::string::npos)
    //}
    //
    //    if(port_list.size()<1)
    //    {
    //        ROS_INFO("there is no ports available \n ");
    //        return 0;
    //    }
